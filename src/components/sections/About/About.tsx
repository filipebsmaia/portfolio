...
export function About({ data }: AboutProps) {
  ...
  const executeCommand = useCallback(
    (cmd: string, currentCwd?: string): { lines: TerminalLine[]; newCwd?: string } => {
      ...
      const promptStr = makePrompt(activeCwd);
      ...
      const output =
        data.shellCommands[resolved] ??
        data.shellCommands[resolved.replace(/\/?$/, '/')] ??
        data.shellCommands[resolved.replace(/\/$/, '')] ??
        data.shellCommands[trimmed] ??
        data.shellCommands[trimmed.replace(/\/?$/, '/')] ??
        data.shellCommands[trimmed.replace(/\/$/, '')];
    ...
    [cwd, data.shellCommands, resolveCommand],
  );
  ...
  useEffect(() => {
    ...
    let currentCwd = '~';
    for (const cmd of data.initialCommands) {
    ...
    setCwd(currentCwd);
  }, [initialized, data.initialCommands, executeCommand]);
  ...
  const handleTab = useCallback(
    (current: string) => {
      ...
      const commands = Object.keys(data.shellCommands);
  ...
  return (
    <section id="about" className={styles.about}>
      <div className={styles.container}>
        <ScrollReveal>
          <SectionHeader fileName="about.md" title="About" />
        </ScrollReveal>

        <ScrollReveal delay={100}>
          <TerminalCard title={`bash - ${USER}`}>
